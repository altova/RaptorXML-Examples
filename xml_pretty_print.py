...
#
# Example invocation:
#   raptorxml valxml-withxsd --streaming=false --script=xml_pretty_print.py ExpReport.xml
...
def _pretty_print(node,output,depth=0):
    ...
    if isinstance(node,xml.ElementInformationItem):
        ...
        for attr in node.namespace_attributes:
            ...
            start_tag.append('%s="%s"' % (attr.local_name, attr.normalized_value))
        ...
        if not node.element_content_whitespace:
            output.append('%s%s' % (' '*depth*3, node.value.replace('&','&amp;').replace('<','&gt;')))
        ...
        pi = []
        pi.append('%s<?%s' % (' '*depth*3, node.target))
        if node.content:
            pi.append(' %s' % node.content)
        pi.append('?>')
        output.append(''.join(pi))

    elif isinstance(node,xml.DocumentInformationItem):
        xml_decl = []
        xml_decl.append('%s<?xml version="%s"' % (' '*depth*3, node.version))
