...
#
# Example invocation:
#   raptorxml valxml-withxsd --streaming=false --script=entry_points.py ExpReport.xml
...
# This entry point will be called by RaptorXML after the DTD validation job (valdtd) has finished (since v2.1)
def on_dtd_finished(job, dtd):
    # dtd object will be None if DTD validation was not successful
    print('on_dtd_finished() was called')

# This entry point will be called by RaptorXML after the XML Schema validation job (valxsd) has finished
...
# This entry point will be called by RaptorXML+XBRL after the XBRL DTS validation job (valdts) has finished
...
# This entry point will be called by RaptorXML+XBRL after the XBRL instance validation job (valxbrl) has finished
