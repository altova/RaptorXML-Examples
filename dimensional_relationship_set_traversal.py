...
#
# Example invocation:
# raptorxmlxbrl valxbrl --script=dimensional_relationship_set_traversal.py
# nanonull.xbrl
...
def print_hypercube(drs, linkrole, hypercube, all, level=1):
    ...
    for rel in drs.hypercube_dimension_relationships(hypercube, linkrole):
        # Access the additional definitionArc attributes using rel.closed and
        # rel.context_element
...
def print_primary_item(drs, linkrole, pi, level=1):
    ...
    for rel in drs.hashypercube_relationships(pi, linkrole):
        print_hypercube(drs, linkrole, rel.target, rel.arcrole ==
                        xbrl.taxonomy.ARCROLE_ALL, level + 1)
