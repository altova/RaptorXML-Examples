...
def print_dts_statistics(dts):
    ...
    # Please note that many taxonomy.DTS properties return generator objects which cannot be used with len() directly

    print('DTS contains %d documents' % sum(1 for _ in dts.documents))
    print('DTS contains %d taxonomy documents' % sum(1 for _ in dts.taxonomy_schemas))
    ...
    # Please note that hypercube and dimension concepts are also members of the xbrli:item substitution group
    print('DTS contains %d item concepts' % (sum(1 for _ in dts.items) - sum(1 for _ in dts.hypercubes) - sum(1 for _ in dts.dimensions)))
    ...
    print('DTS contains %d formula parameters' % sum(1 for _ in dts.parameters))
...
def print_instance_statistics(instance):
    ...
    # Please note that many xbrl.Instance properties return generator objects which cannot be used with len() directly
    ...
    print('Instance contains %d top-level item facts' % len(instance.child_items))
    print('Instance contains %d top-level tuple facts' % len(instance.child_tuples))
    ...
    print('Instance contains %d facts with attached footnotes' % facts_with_footnotes)
    ...
    for lang in footnotes.keys():
        print('Instance contains %d footnote resources in language %s' % (footnotes[lang],lang))
