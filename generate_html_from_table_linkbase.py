...
# --script-param="elimination:true" nanonull.xbrl
    ...
    if cs:
        for aspect in cs.values():
            if isinstance(aspect, xbrl.ConceptAspectValue):
                html.append('<p class="label">%s</p>\n' % concept_label(
                    aspect.concept, header.structural_node.preferred_label, lang))
...
def generate_table_caption(html, table, z, lang=None):
    ...
    for header in table.axis(Z).slice(z):
        breakdown = table.axis(Z).structural_breakdown(
            header.row).definition_breakdown
        if previous_breakdown != breakdown:
            previous_breakdown = breakdown
            labels = list(breakdown.labels(lang=lang))
            if len(labels):
                html.append('<p class="label">%s</p>\n' % labels[0].text)
    ...
    for row in range(x_axis.row_count):
        ...
        for header in x_axis.row(row):
            if header.structural_node.is_rollup():
                ...
                rowspan = x_axis.row_count - \
                    header.row if header_with_only_rollup_children(
                        header) else 1
                html.append('<th colspan="%d" rowspan="%d">\n' %
                            (header.span, rowspan))
                generate_label(html, header, lang)
...
def generate_table_body(html, table, y_range, z, lang=None):
    ...
    for y in y_range:
        ...
        for header in y_axis.slice(y):
            # Only generate <th> elements in rows where the header cell starts
            # a vertical span
            if header.slice == y:
                if header.structural_node.is_rollup():
                    if not header.parent.structural_node.is_rollup() and not header_with_only_rollup_children(header.parent):
...
def generate_table(job, instance, deftable, params):
    single_output_file = job.script_params.get(
            ...
            if not table.is_empty():
                for z in range(table.axis(Z).slice_count):
                    for y in range(0, table.axis(Y).slice_count, max_rows):
                        ...
                        generate_table_body(body, table, range(
                            y, min(y + max_rows, table.axis(Y).slice_count)), z, lang)
                        body.append('</table>\n')
                        if not single_output_file:
                            write_html(job, '%s_%d_%d_%d.html' % (
                                deftable.id, table_idx, z, y / max_rows), body)
                            body = []
            else:
                body.append(
                    '<p class="error">Table %s is empty (no data found)!</p>\n' % deftable.id)
                if not single_output_file:
                    write_html(job, deftable.id + '.html', body)
                    body = []
            table_idx += 1
