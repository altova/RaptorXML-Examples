...
#
# Example invocation:
# raptorxml valxml-withxsd --streaming=false --script=xml_as_html.py
# ExpReport.xml
...
def _xml_to_html(node, html, depth=0):
    ...
    if isinstance(node, xml.ElementInformationItem):
        ...
        if node.prefix:
            html.append(
                '<span class="elem-prefix">%s</span><span class="elem-prefix-sep">:</span>' % node.prefix)
        ...
        for attr in node.namespace_attributes:
            ...
            if attr.prefix:
                html.append(
                    '<span class="nsattr-prefix">%s</span><span class="nsattr-prefix-sep">:</span>' % attr.prefix)
            html.append('<span class="nsattr-name">%s</span>' %
                        attr.local_name)
            html.append('<span class="nsattr-op">=&quot;</span>')
            html.append('<span class="nsattr-value">%s</span>' %
        ...
        for attr in node.attributes:
            ...
            if attr.prefix:
                html.append(
                    '<span class="attr-prefix">%s</span><span class="attr-prefix-sep">:</span>' % attr.prefix)
            html.append('<span class="attr-name">%s</span>' % attr.local_name)
            html.append('<span class="attr-op">=&quot;</span>')
            html.append('<span class="attr-value">%s</span>' %
                        attr.normalized_value)
            html.append('<span class="attr-op">&quot;</span>')
        ...
        if not node.element_content_whitespace:
            html.append('<span class="indent">%s</span>' % ' ' * depth * 3)
            html.append('<span class="cdata">%s</span></br>' %
        ...
        html.append('<span class="indent">%s</span>' % ' ' * depth * 3)
        html.append(
            '<span class="pi-op">&lt;?</span><span class="pi-target">%s</span>' % node.target)
        if node.content:
        ...
        html.append('<span class="indent">%s</span>' % ' ' * depth * 3)
        html.append(
            '<span class="pi-op">&lt;?</span><span class="pi-target">xml</span><span class="pi-content"> version="%s"<span>' % node.version)
        if node.character_encoding_scheme:
            html.append('<span class="pi-content"> encoding="%s"</span>' %
                        node.character_encoding_scheme)
        if node.standalone:
            html.append(
                '<span class="pi-content"> standalone="%s"</span>' % node.standalone)
        html.append('<span class="pi-op">?&gt;</span></br>')
